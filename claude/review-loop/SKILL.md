---
name: review-loop
description: Use when the user asks for a review-loop, deep review, architecture review, second-opinion review, review-fix loop, acceptance review, or wants findings implemented with tests/checks. Orchestrates standard, deep, and architecture review modes that fix all confirmed in-scope findings, write standalone artifacts to a new versioned file, then repeat primary and second-opinion review (Codex CLI or fresh-context subagent) until no actionable issues remain or a Human Gate blocks.
---
...
versioned artifact or local fixes, checks, primary re-review, second-opinion
acceptance, and learning capture.
Stop: Stop before gated actions, when primary and second-opinion reviewers have
no confirmed actionable findings, after three repeated blockers, or when the
next step is a broad design decision.
...
2. Run the primary local review yourself (fresh eyes; use read-only subagents
   for focused axes where useful).
3. Run a second-opinion review through
   [second-opinion-protocol.md](references/second-opinion-protocol.md).
   Prefer an installed `codex` CLI as the independent reviewer. If Codex is
   missing or blocked, document the exact blocker, then use a fresh-context
   read-only subagent as the fallback; never simulate the external reviewer.
...
9. Repeat the loop until primary and second-opinion both have no confirmed
...
- Second-opinion phase: use the Codex CLI first through
  [second-opinion-protocol.md](references/second-opinion-protocol.md); use a
  fresh-context subagent only as a documented fallback.
...
- UI, course, website, or app review: use browser/Playwright checks.
...
ambiguous, or deep. Read
[second-opinion-protocol.md](references/second-opinion-protocol.md) before
invoking the second-opinion reviewer. Read
...
5. Build and run the second-opinion phase with the Codex CLI first.
   Stop at the Privacy/Human Gate unless the user already gave explicit Go for
   the scoped packet. If Codex is unavailable or blocked, record that
   explicitly before using the subagent fallback.
...
10. Verify with the feedback signal from step 3, then run primary and
    second-opinion re-review on the updated result.
