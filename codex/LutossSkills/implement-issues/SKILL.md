...
From the repo root, run the helper scripts that ship in this skill's `scripts/` directory. Codex skills usually live in `$HOME/.agents/skills/` (user scope) or `.agents/skills/` (repo scope) — use the absolute path of this skill folder.

```bash
node "$HOME/.agents/skills/implement-issues/scripts/list-ready-issues.mjs" .scratch/<feature-slug>
node "$HOME/.agents/skills/implement-issues/scripts/build-agent-prompt.mjs" .scratch/<feature-slug>
node "$HOME/.agents/skills/implement-issues/scripts/build-agent-prompt.mjs" .scratch/<feature-slug> --issue .scratch/<feature-slug>/issues/01-example.md
```

PowerShell equivalent:

```powershell
node "$env:USERPROFILE/.agents/skills/implement-issues/scripts/list-ready-issues.mjs" .scratch/<feature-slug>
```

If the skill is installed elsewhere, use that absolute path.
...
4. Loop until there are no AFK issues left:
   ...
   - Spawn as many worker subagents as are useful for that batch, usually one worker per issue.
...
If no AFK issues remain, report `<promise>NO MORE TASKS</promise>` in the runner context or tell the user directly in normal Codex conversation.
...
- Parallelize all issues in the current batch when the work scopes are independent or each worker runs in an isolated subagent workspace.
