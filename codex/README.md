...
10 skills for OpenAI Codex (CLI, IDE extension, app).
...
**Skills:** `agent-evals`, `agent-orchestration`, `ask-claude`, `implement-issues`, `improve-project-structure`, `loop-creator`, `project-review`, `review-loop`, `verify-before-done`, `youtube-transcript`.
...
- `youtube-transcript` needs Python 3 plus `ffmpeg` on PATH for the optional frame cards; `yt-dlp` and Pillow are auto-installed by the bundled scripts. Run its scripts with network approval — the sandbox blocks network by default.
