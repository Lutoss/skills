...
- Codex's sandbox blocks network access by default — the yt-dlp calls need it,
  so run the scripts with escalated permissions (approval) instead of silently
  letting them fail. On Windows call `python`, elsewhere `python3`.
- Typical timing: metadata+comments+captions 15–60 s, the frames step 1–4 min
  depending on video length — for long videos start it and check back rather
  than blocking.
